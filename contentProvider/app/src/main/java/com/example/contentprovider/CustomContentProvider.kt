...
class CustomContentProvider: ContentProvider() {
    ...
    val CREATE_TABLE = "CREATE TABLE $TABLE_NAME(id INSERT PRIMARY KEY AUTOINCERMENT, " +
            "name TEXT NOT NULL, nickname TEXT NOT NULL;"
    init {
        mUriMatcher = UriMatcher(UriMatcher.NO_MATCH)
        mUriMatcher?.addURI(PROVIDER_NAME, "nicknames", NICKNAME)
        mUriMatcher?.addURI(PROVIDER_NAME, "nikenames /#", NICKNAME_ID)
    }
    ...
    override fun update(
        ...
        when (mUriMatcher?.match(uri)) {
            ...
            NICKNAME_ID -> {
                ...
                count = mDatabase?.update(TABLE_NAME, values, "$ID = ${uri?.lastPathSegment}" +
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<out String>?): Int {
        ...
        when (mUriMatcher?.match(uri)) {
            NICKNAME -> count = mDatabase?.delete(TABLE_NAME, selection,
                selectionArgs)!!
            NICKNAME_ID -> {
                var whereCLause = ""
                if (!TextUtils.isEmpty(selection)) {
                ...
                count = mDatabase?.delete(TABLE_NAME, "ID = ${uri?.lastPathSegment}" +
                        "$whereCLause", selectionArgs)!!
            }
            else -> throw IllegalArgumentException("Unsupported URI" + uri)
        }
